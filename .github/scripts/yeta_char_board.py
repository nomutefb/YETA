...
산출 = `viewer/assets/yeta_char/board/<slug>_sheet<A|B|C>[_v<n>].png`(+webp 768w) = git 정본(운영자 편집·칸 자르기 베이스).
...
SHEET = (os.environ.get("YETA_BOARD_SHEET") or "A").strip().upper()      # A = 감정 9칸(3×3 · 앱 정합) · B = 감정 20칸(5×4) · C = 포즈 12칸(4×3 · 표정 고정)
...
# 시트 비례 = 칸 3:4 기준 — A(3열×3행) → 세로 3:4 / B(5열×4행)·C(4열×3행) → 정사각. gpt-image 허용 size 3종 중 근사치.
SIZES = {"A": "1024x1536", "B": "1024x1024", "C": "1024x1024"}
...
def main():
    ...
    if SHEET not in SIZES:
        print(f"⚠️ 알 수 없는 시트 '{SHEET}' — A · B · C 중 하나"); return 1
