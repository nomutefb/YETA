...
L += [f"[씨앗 후보] 줄마다 하나씩 · {pool}개. **딱 하나만 골라 사건의 축으로 삼아라** — 나머지는 통째로 버려라(비트마다 다른 씨앗을 나눠 쓰면 그게 나열이다). 대화 결에 걸리는 게 없으면 씨앗 없이 지금 장소에서 눈앞에 보일 법한 것 하나로 가라:"]
for i, sd in enumerate(seeds, 1): L.append(f"{i}) {sd}")
L += ["", "[규칙]",
      f"- 사건은 **하나뿐**이다. 정확히 {batch}줄, 한 줄 = 그 사건의 한 비트. 번호·불릿·따옴표·설명 금지(줄만 뱉어라).",
      "- 비트 순서 = 뿌림 → 굴러감 → 회수. 1줄 = 터진 순간만. 2줄 = 그게 번져 딴 사람·딴 자리를 끌어들인다. 3줄 = 치워지고 흔적 하나가 남는다.",
      "- **줄마다 사물이나 소리 하나를 물려라**: 1줄에 심은 것(엎어진 국물통·굴러간 캔·나간 간판불)이 2줄에서 남의 손에 넘어가고 3줄에서 회수된다. 그 물건이 사건을 꿰는 실이다.",
      ...
      f"- {batch}줄은 서로 다른 순간·다른 각도다. 같은 문장 구조·같은 감각(소리면 소리)을 두 번 쓰지 마라.",
...
- 길이(운영자 260714 확정 · 실측검증): 대사는 **1~3문장, 기본 80자 안**에서 끝내라 — 감정이 깊게 흐르는 장면만 120자까지. 단 narration 카드의 여운 장면·해금 에피소드(관계 단계가 여는 순간)는 지문 최대 2줄을 더 허용한다(그 순간만 숨 쉬게 · 평의회 260714). 대사가 종결어미로 닫혔는데 감정이 남은 턴(`<<OPEN:>>`을 다는 턴·공기가 base가 아닌 턴)은 그 뒤에 **지문 한 줄**로 공기를 깔아라 — 대사를 풀어 설명하지 말고 눈에 보이는 것 하나만(카드 narration이 false여도 이 한 줄은 된다 · 매 턴은 아니다 · 운영자 260725). 카드 예시 대화의 말투·어미는 복제하되 **길이는 이 계약이 우선**(예시의 절반 호흡). 하고 싶은 말이 남으면 다음 턴을 위해 아껴라 — 대화는 캐치볼이다.
